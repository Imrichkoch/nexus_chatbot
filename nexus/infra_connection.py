...
WINDOWS_USER_PATTERN = re.compile(r'^(?:[A-Za-z0-9_.-]{1,63}\\)?[A-Za-z_][A-Za-z0-9_.@-]{0,127}$')
FILE_PATTERN = re.compile(r'^[A-Za-z0-9][A-Za-z0-9_.-]{0,127}$')
REMOTE_COLLECTOR = '/usr/local/bin/nexus-infra-readonly'
WINDOWS_COLLECTOR = r'C:\ProgramData\NexusChat\nexus-infra-readonly.ps1'
...
class InfraConnectionPayload(BaseModel):
    kind: Literal['linux_ssh', 'windows_ssh', 'windows_winrm'] = 'linux_ssh'
    name: str = Field(min_length=1, max_length=80)
    host: str = Field(min_length=1, max_length=253)
    port: int = Field(default=22, ge=1, le=65535)
    username: str = Field(min_length=1, max_length=192)
    identity_file: str = Field(default='', max_length=128)
    password: str = Field(default='', max_length=1000, exclude=True)
    auth: Literal['ntlm', 'basic'] = 'ntlm'
    ca_file: str = Field(default='', max_length=128)
...
def _validated_response(stdout: str):
...
def collect_remote_infra(settings: InfraConnectionPayload, mode: str, *, key_root: Path, known_hosts: Path):
    ...
    return _validated_response(result.stdout)


def collect_windows_winrm(settings: InfraConnectionPayload, mode: str, *, ca_root: Path):
    if mode not in {'snapshot', 'live'}:
        raise InfraConnectionError('Unsupported infrastructure source mode.')
    if not settings.password:
        raise InfraConnectionError('A WinRM password is required.')
    ca_path = None
    if settings.ca_file:
        ca_path = str(_approved_file(ca_root, settings.ca_file))
    try:
        import winrm
        session = winrm.Session(
            f'https://{settings.host}:{settings.port}/wsman',
            auth=(settings.username, settings.password),
            transport=settings.auth,
            server_cert_validation='validate',
            ca_trust_path=ca_path,
            read_timeout_sec=15,
        ...
        result = session.run_cmd(
            ...
            ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'AllSigned',
             '-File', WINDOWS_COLLECTOR, '-Mode', mode],
        )
    except Exception as error:
        raise InfraConnectionError('The Windows WinRM collector could not be reached or rejected access.') from error
    if result.status_code:
        raise InfraConnectionError('The Windows WinRM read-only collector rejected the request.')
    return _validated_response(result.std_out.decode('utf-8-sig', errors='strict'))
...
class InfraConnections:
    def __init__(self, path: Path, *, local_snapshot_path: str,
                 ...
                 known_hosts: Path, secret_root: Path | None = None,
        ...
        self.secret_root = secret_root or path.parent / 'infra-winrm-secrets'
        self.ca_root = ca_root or path.parent / 'infra-ca'
        self.remote_collector = self._collect_remote
    ...
    def _collect_remote(self, settings, mode):
        if settings.kind == 'windows_winrm':
            return collect_windows_winrm(settings, mode, ca_root=self.ca_root)
        return collect_remote_infra(
            settings, mode, key_root=self.key_root, known_hosts=self.known_hosts)

    def _secret_path(self, connection_id):
        return self.secret_root / connection_id

    def _read_secret(self, connection_id):
        try:
            return self._secret_path(connection_id).read_text(encoding='utf-8')
        except OSError:
            return ''

    def _write_secret(self, connection_id, password):
        self.secret_root.mkdir(parents=True, exist_ok=True)
        descriptor, temporary = tempfile.mkstemp(prefix='.winrm-', dir=self.secret_root)
        try:
            with os.fdopen(descriptor, 'w', encoding='utf-8') as handle:
                ...
                os.fsync(handle.fileno())
            os.chmod(temporary, 0o600)
            os.replace(temporary, self._secret_path(connection_id))
        finally:
            Path(temporary).unlink(missing_ok=True)

    @staticmethod
    def _validate(payload, *, password_configured=False):
        if not payload.name.strip():
            raise InfraConnectionError('A server connection name is required.')
        user_pattern = WINDOWS_USER_PATTERN if payload.kind.startswith('windows_') else USER_PATTERN
        if not HOST_PATTERN.fullmatch(payload.host) or not user_pattern.fullmatch(payload.username):
            raise InfraConnectionError('Enter a valid server host and username.')
        if payload.kind in {'linux_ssh', 'windows_ssh'}:
            if not FILE_PATTERN.fullmatch(payload.identity_file):
                raise InfraConnectionError('Use only an approved SSH key filename, not a path.')
        elif not payload.password and not password_configured:
            raise InfraConnectionError('A WinRM password is required.')
        if payload.ca_file and not FILE_PATTERN.fullmatch(payload.ca_file):
            raise InfraConnectionError('Use only an approved CA certificate filename, not a path.')

    def _public(self, connection_id, profile):
        value = {'kind': 'linux_ssh', **profile, 'id': connection_id}
        value['password_configured'] = self._secret_path(connection_id).is_file()
        return value
    ...
    def _draft(self, payload, connection_id=None):
        ...
        password_configured = bool(connection_id and self._secret_path(connection_id).is_file())
        self._validate(payload, password_configured=password_configured)
        ...
        if connection_id and self.bound_count(connection_id) and any(
            current.get(key, 'linux_ssh' if key == 'kind' else None) != getattr(payload, key)
            for key in ('kind', 'host', 'port', 'username')
        ):
    ...
    def test(self, payload, connection_id=None, mode='live'):
        ...
        if draft.kind == 'windows_winrm' and not draft.password and connection_id:
            draft = draft.model_copy(update={'password': self._read_secret(connection_id)})
    ...
    def save(self, payload, connection_id=None):
        ...
        with self.lock:
            ...
            if payload.kind == 'windows_winrm':
                if payload.password:
                    self._write_secret(connection_id, payload.password)
            else:
                self._secret_path(connection_id).unlink(missing_ok=True)
    ...
    def collect(self, connection_id, mode):
        ...
        with self.lock:
            ...
            if settings.kind == 'windows_winrm':
                settings = settings.model_copy(update={'password': self._read_secret(connection_id)})
